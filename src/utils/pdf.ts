...
const drawGuideLines = (sheet: PDFPage, paperW: number, paperH: number, slotsPerSheet: 2 | 4, settings: BookletSettings) => {
  ...
  const mark = mmToPt(10);
  const guideColor = rgb(0.18, 0.18, 0.18);
  const guideOpacity = 0.25;
  const guideDash = [2, 2];

  if (settings.foldGuides) {
    sheet.drawLine({ start: { x: centerX, y: paperH - mark }, end: { x: centerX, y: paperH - 2 }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
    sheet.drawLine({ start: { x: centerX, y: 2 }, end: { x: centerX, y: mark }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
  }

  if (settings.cutGuides && slotsPerSheet === 4) {
    sheet.drawLine({ start: { x: centerX - mark, y: paperH - 2 }, end: { x: centerX + mark, y: paperH - 2 }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
    sheet.drawLine({ start: { x: centerX - mark, y: 2 }, end: { x: centerX + mark, y: 2 }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
    sheet.drawLine({ start: { x: 2, y: centerY - mark }, end: { x: 2, y: centerY + mark }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
    sheet.drawLine({ start: { x: paperW - 2, y: centerY - mark }, end: { x: paperW - 2, y: centerY + mark }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
  }

  if (settings.stitchGuides && slotsPerSheet === 2) {
    [paperH * 0.2, paperH * 0.5, paperH * 0.8].forEach((y) => {
      sheet.drawLine({ start: { x: centerX - mmToPt(4), y }, end: { x: centerX + mmToPt(4), y }, thickness: 0.5, color: guideColor, opacity: guideOpacity, dashArray: guideDash, dashPhase: 0 });
    });
  }
};
